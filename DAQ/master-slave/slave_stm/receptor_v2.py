...
SPI_DEVICE    = 0           # CE0 (GPIO8) → conectar al NSS del STM32
SPI_SPEED_HZ  = 1_000_000   # 1 MHz — bajar a 500_000 si hay errores
SPI_MODE      = 0b00        # CPOL=0, CPHA=0 — debe coincidir con STM32
...
# ── Modo de validación ────────────────────────────────────────────
#
#  "estricto"  → sync byte + checksum deben estar bien
#                (usar en producción, en pista)
#
#  "sync"      → solo verifica el byte 0xAA, ignora checksum
#                (útil cuando el STM32 aún no calcula bien el CS)
#
#  "ninguno"   → acepta todo lo que llegue sin verificar nada
#                (debug inicial: para ver qué bytes están llegando)
#
...
CSV_HEADER = [
    ...
    "dx4_mm",
    "dx5_mm",
    "checksum_ok",   # 1 = CS válido, 0 = CS falló (dato guardado igual)
]
...
def calcular_checksum(raw: bytes) -> tuple:
    """Devuelve (checksum_calculado, checksum_recibido)."""
    cs = 0
    for b in raw[:-1]:
        cs ^= b
    return cs, raw[-1]
...
def parsear_paquete(raw: bytes) -> tuple:
    ...
    Devuelve (datos_dict, estado_str):
      datos_dict → dict con los campos, o None si se descarta
      estado_str → "ok" | "ok_sin_cs" | "sin_sync" | "longitud" | "unpack_error"

    Si MODO_VALIDACION != "estricto", los paquetes con checksum malo
    se aceptan igual con estado "ok_sin_cs" y se guardan en CSV.
    """
    ...
    # ── Decisión según modo ───────────────────────────────
    if MODO_VALIDACION == "estricto":
        if raw[0] != SYNC_BYTE:
            return None, "sin_sync"
        if not checksum_ok:
            return None, f"checksum  esperado={cs_calc:#04x}  recibido={cs_rx:#04x}"

    elif MODO_VALIDACION == "sync":
        if raw[0] != SYNC_BYTE:
            return None, "sin_sync"
        # checksum no bloquea pero se registra en la columna checksum_ok del CSV

    elif MODO_VALIDACION == "ninguno":
        pass  # pasa todo — solo puede fallar el unpack

    # ── Desempaquetar ─────────────────────────────────────
    # < = little-endian | 7f = 7 floats | I = uint32
    try:
        angulo, acel, dx1, dx2, dx3, dx4, dx5, tiempo_us = struct.unpack_from(
            "<7fI", raw, 1   # offset 1: saltar el sync byte
        )
...
def mostrar_consola(datos: dict, t_pc_ms: float, n_muestra: int,
                    n_errores: int, n_sin_cs: int ):
    """Imprime los datos del paquete con formato legible."""
    ...
    color_desc  = VERDE if tasa_error  < 1.0 else (AMARILLO if tasa_error  < 5.0 else ROJO)
    color_cs    = VERDE if tasa_sin_cs < 5.0 else AMARILLO

    print(f"\n  Descartados (sin sync): {color_desc}{n_errores} ({tasa_error:.1f}%){RESET}")
...
def main():
    ...
    n_muestra  = 0
    n_errores  = 0   # paquetes descartados (sin sync o longitud incorrecta)
    n_sin_cs   = 0   # paquetes guardados con checksum incorrecto
    ...
    try:
        while True:
            ...
            # ── Pedir 34 bytes al STM32 ───────────────────
            # xfer2 mantiene CS activo durante toda la transferencia.
            # Los 0x00 generan el clock; el STM32 responde en MISO.
            ...
            if datos is None:
                # Paquete descartado completamente
                n_errores += 1
                print(f"\r  {AMARILLO}[!] Descartado #{n_muestra}: {estado}"
                      f"  raw[0]={raw[0]:#04x}{RESET}",
                      end="", flush=True)
            else:
                # Paquete aceptado (puede tener CS malo en modo no-estricto)
                ...
                # ── Mostrar ───────────────────────────────
                mostrar_consola(datos, t_pc_ms, n_muestra, n_errores, n_sin_cs)

            # ── Mantener período de 20 ms ─────────────────
