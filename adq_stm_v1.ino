...
bool esTransicionValida(int desde, int hacia) {
  if (desde == -1) return true;   // arranque, estado desconocido: acepta el primero
  if (desde == hacia) return true;
  switch (desde) {
    case 0: return (hacia == 1 || hacia == 2);               // Neutro <-> 1ra o 2da
    case 1: return (hacia == 0 || hacia == 2);               // 1ra <-> N o 2da
    case 2: return (hacia == 0 || hacia == 1 || hacia == 3); // 2da <-> N, 1ra o 3ra
...
int leer_cambio_confirmado(int valor_adc) {
  ...
  if (contador_confirmacion >= NUM_CONFIRMACIONES) {
    if (esTransicionValida(estado_actual, candidato)) {
      estado_actual = candidato;
    }
    // si no es vecino válido, se descarta como ruido y se mantiene estado_actual
  }
