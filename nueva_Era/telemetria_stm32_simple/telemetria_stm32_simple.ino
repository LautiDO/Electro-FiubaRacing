...
//====================================================
#define MAX_DATOS 65   // 1 sync + 15×4 float + 1×4 int32 (cambio) — sin tiempo_us
...
void armarPaquete(float ang, float acel,
  ...
  memcpy(&buf[pos], &cambio,  4); // pos = 65
}
...
int clasificar_lectura(int valor) {
  ...
  if (valor >= (neutro - TOLERANCIA) && valor <= (neutro + TOLERANCIA)) {
    if (estado_actual == 1 || estado_actual == 2) return 0;
    return -1;
  }
