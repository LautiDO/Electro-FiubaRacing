...
STM32_TAM_PKT  = 13          # 1 sync + 1 int32 (cambio) + 1 float (angulo) + 1 uint32 (tiempo_us)
STM32_FORMATO  = '<ifI'      # little-endian: cambio (int32), angulo (float), tiempo_us (uint32)
...
def _parsear_paquete_stm32(raw):
    if len(raw) != STM32_TAM_PKT or raw[0] != STM32_SYNC:
        return None
    try:
        cambio, ang, t_us = struct.unpack_from(STM32_FORMATO, raw, 1)
    except struct.error:
        return None
    return {
        "marcha":        cambio,
