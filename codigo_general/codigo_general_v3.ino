...
void loop() {
 ...
 //-----------------------Llamar funciones-------------------------
    temperatura_motor = medir_temp_motor(pin_temp_motor , 180 , 5000);    
    ...
    guardar_datos(AFR ,temperatura_motor);
...
float medir_temp_motor(int pin_motor, float Rmin , float Rmax){
    ...
    float vref = (adc_val * Vcc) / 4095.0; //lo paso a tension
  
    Serial.print("vref: ");
    Serial.println(vref);
    
    // Calcular resistencia del termistor
    float Rt = (vref * R_fija) / (Vcc - vref);

    Serial.print("rt: ");
    Serial.println(Rt);
    
    //map(potValor, potMin, potMax, 0, 180);
    float temp_motor = map(vref , 2.5 , 3.5, 28 ,100); // segun hoja de datos el sensor te entrega entre esas dos tensiones 2.5 a 3.5
