...
Fuentes de datos:
  - STM32   → SPI       (marcha, ángulo, acel_lateral, dx1..dx5, presiones STM, tiempo)
...
# Estructura real del paquete armado en armarPaquete() del .ino (45 bytes):
#   [0]      0xAA          sync
#   [1..4]   angulo        float
...
#   [21..24] dx4           float
#   [25..28] dx5           float
#   [29..32] presion1      float   (medida por el STM32)
#   [33..36] presion2      float   (medida por el STM32)
#   [37..40] cambio        int32
#   [41..44] tiempo_us     uint32
STM32_TAM_PKT  = 65          # 1 sync + 14 floats (4B c/u) + 1 int32 + 1 uint32
STM32_FORMATO  = '<14fiI'    
...
def _parsear_paquete_stm32(raw):
    """
    Decodifica el paquete de 65 bytes del STM32:
      [0]       sync (0xAA)
      [1..4]    angulo        float
      [5..8]    acel_x        float
      [9..12]   acel_y        float
      ...
      [41..44]  dx4           float
      [45..48]  dx5           float
      [49..52]  presion1      float
      [53..56]  presion2      float
      [57..60]  cambio        int32
      [61..64]  tiempo_us     uint32
    """
...
def mostrar_consola(stm, ecu, gps, t_ms, n_muestras, n_errores, n_resync):
    ...
    # --- Acelerómetros / Giroscopio ---
    print(f"  {'Acel (X/Y/Z)':<22}  {_fmt(stm['acel_x'], 2)} / {_fmt(stm['acel_y'], 2)} / {_fmt(stm['acel_z'], 2)}  g")
    print(f"  {'Giro (X/Y/Z)':<22}  {_fmt(stm['giro_x'], 1)} / {_fmt(stm['giro_y'], 1)} / {_fmt(stm['giro_z'], 1)}  °/s")
    ...
    try:
        while True:
            ...
            csv_writer.writerow({
                ...
                "marcha":              stm["marcha"],
                "angulo":              stm["angulo"],   
                "acel_x":              round(stm["acel_x"], 4),
                "acel_y":              round(stm["acel_y"], 4),
                "acel_z":              round(stm["acel_z"], 4),
                "giro_x":              round(stm["giro_x"], 3),
                "giro_y":              round(stm["giro_y"], 3),
                "giro_z":              round(stm["giro_z"], 3),
                "dx1":                 round(stm["dx1"], 2),
                "dx2":                 round(stm["dx2"], 2),
                "dx3":                 round(stm["dx3"], 2),
                "dx4":                 round(stm["dx4"], 2),
                "dx5":                 round(stm["dx5"], 2),        
                "stm_presion1":        round(stm["stm_presion1"], 2),
                "stm_presion2":        round(stm["stm_presion2"], 2),
                "rpm":                 ecu["rpm"],
                "tps":                 ecu["tps"],
